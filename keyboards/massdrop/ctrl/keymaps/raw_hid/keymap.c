...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    [1] = LAYOUT(
        ...
        _______,RGB_RMOD, RGB_VAD, RGB_MOD, RGB_HUD, RGB_SAD, DBG_TOG, _______, DBG_KBD, DBG_MOU, _______, _______, _______, \
        _______, RGB_TOG, _______, _______, _______, MD_BOOT, NK_TOGG,DBG_MTRX, _______, _______, _______, _______,                              _______, \
...
//     print("Spamming console");
// #endif
};

// Runs constantly in the background, in a loop.
void matrix_scan_user(void) {
    if (!is_led_off && timer_elapsed32(time_last_activity) > TIMEOUT_ACTIVITY) {
...
void raw_hid_lights_toggle(void) {
    change_led_state(!is_led_off);

    raw_hid_buffer[1] = CTRL_HID_OK;
    raw_hid_buffer[2] = (uint8_t) is_led_off;
...
void raw_hid_rgbmatrix_mode(uint8_t *data) {
    ...
    if (mode >= RGB_MATRIX_EFFECT_MAX) {
        ...
        raw_hid_buffer[3] = mode;
        raw_hid_buffer[4] = CTRL_HID_EOM;
        return;
    }
    rgb_matrix_mode_noeeprom(mode);

    raw_hid_buffer[1] = CTRL_HID_OK;
    raw_hid_buffer[2] = RGB_MATRIX_EFFECT_MAX;
    raw_hid_buffer[3] = mode;
    raw_hid_buffer[4] = CTRL_HID_EOM;
}
