...
/** Logic for saving/loading lighting settings to EEPROM **/
...
/** Refactored code for changing settings that syncs to EEPROM **/
...
void led_set_animation_direction(bool direction) {
    kb_config.led_animation_direction = direction;
    sync_settings();
}
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
   ...
   uprintf("KL: kc: 0x%04X, col: %u, row: %u, pressed: %b, time: %u, interrupt: %b, count: %u\n", keycode, record->event.key.col, record->event.key.row, record->event.pressed, record->event.time, record->tap.interrupted, record->tap.count);
    ...
    switch (keycode) {
        ...
        case L_T_PTD:
            if (record->event.pressed) {
                led_set_animation_direction(!led_animation_direction);
            }
            return false;
        case U_T_AGCR:
